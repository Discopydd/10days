...
void GimmickDemoScene::Initialize() {
...
void GimmickDemoScene::Finalize() {
	delete playerModel_;
	delete blockModel_;
	delete switchModel_;
	delete floorModel_;
...
void GimmickDemoScene::InitializeTransform(
...
Vector3 GimmickDemoScene::UpdatePlayer() {
	...
	// デモ用なので、プレイヤーだけはステージ外へ出ないよう簡易制限する
...
void GimmickDemoScene::UpdateConnection() {
...
void GimmickDemoScene::UpdateSwitch() {
	// すでに固定済みなら判定不要
...
void GimmickDemoScene::UpdateRope() {
	if (!movableBlock_.IsConnected()) {
		return;
	}
...
void GimmickDemoScene::ResetDemo() {
	playerWorldTransform_.translation_ =
		kPlayerStartPosition_;
